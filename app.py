...
1. Enter the systematic or common <strong>Gene Name</strong>.<br>
...
4. Repair templates are centered and both are provided <strong>5' to 3'</strong> for ordering.
</div>
""", unsafe_allow_html=True)
...
if run:
    ...
    if g_id:
        ...
        if m_idx+3 > c_end: st.error("Residue out of bounds.")
            ...
            if not sites:
                st.warning(f"No PAM sites found within 100bp of residue {res}.")
            ...
            for i, site in enumerate(sites[:5]):
                with st.container():
                    ...
                    pam_pos = site['pos'] + 21 if site['strand'] == 'forward' else site['pos'] + 1
                    center_point = (m_idx + pam_pos) // 2
                    half = t_len // 2
                    raw_start = max(0, center_point - half)
                    v_s = off + (((raw_start - off) // 3) * 3)
                    v_e = v_s + t_len
                    wt_dna = f_seq[v_s:v_e]
                    r_mut = m_idx - v_s
                    
                    m_dna_l = list(wt_dna)
                    if 0 <= r_mut <= len(m_dna_l)-3:
                    ...
                    if not is_bk:
                        ...
                        for p in reversed(scan):
                            cs = (((v_s + p - off) // 3) * 3) + off - v_s # Joined line
                    ...
                    # SYNCED CASING & 5' to 3' REVERSE COMPLEMENT
                    ...
                    rc_dna_raw = str(Seq(m_dna_f).reverse_complement())
                    t_len_curr = len(m_dna_f)
                    rc_mut_indices = [(t_len_curr - 1 - idx) for idx in mut_indices]
                    dis_dna_rc = "".join([c.lower() if idx in rc_mut_indices else c.upper() for idx, c in enumerate(rc_dna_raw)])
                    ...
                    for idx, char in enumerate(dis_dna_sense):
                        cl = ' class="mut-site"' if idx in range(r_mut, r_mut+3) else (' class="silent-site"' if idx in mut_indices else '')
                        h += f'<td{cl}>{char}</td>'
                    ...
                    txt += f"Repair (Sense 5'-3'): {dis_dna_sense}\n"
                    txt += f"Repair (RevComp 5'-3'): {dis_dna_rc}\n\n"
                    txt += f"--- PROOFREADING ALIGNMENT ---\nWT PROT:  {wt_p_str}\nWT DNA:   {wt_dna.upper()}\nMUT DNA:  {dis_dna_sense}\nMUT PROT: {mu_p_str}"
